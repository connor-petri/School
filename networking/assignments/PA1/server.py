...
def get_status_line(code: int) -> str:
    """
...
def handle_connection_non_persistent(conn: socket):
    """
...
def handle_connection_persistent(conn: socket):
    """
    ...
    with conn:
        ...
        while True:
            try:
                ...
                if parsed is None:
                    conn.sendall(build_http_response(400, b'', {"Connection": "close"}))
                    return

                if parsed["method"] != "GET":
                    conn.sendall(build_http_response(405, b'', {"Connection": "close"}))
                    return
                ...
                conn.sendall(build_http_response(code, content, {
                    ...
                    "Connection": "close" if wants_close else "keep-alive"
                    }))
