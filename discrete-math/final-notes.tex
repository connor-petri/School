...
\textbf{Counting and Probability}\\
- \textbf{Probability Formula:} $P(E) = \frac{N(E)}{N(S)}$ \\
- $\exists m, n \in \mathbb{Z} \ni m \leq n$, there are $n - m + 1$ integers in the set $\{m, m+1, \dots, n\}$.\\
- \textbf{Multiplication Rule:} If a process can be broken down into $k$ stages where each stage has $n_i$ outcomes,
then the total number of outcomes is $n_1 \cdot n_2 \cdots n_k$.\\
- \textbf{Permutation:} The number of ways to arrange $n$ distinct objects is $n!$.\\
- \textbf{r-Permutation:} The number of ways to arrange $r$ objects from a set of $n$ distinct objects is $nPr = \frac{n!}{(n-r)!}$.\\
- $|A \cup B| = |A| + |B| - |A \cap B|$\\
- $|A \cap B| = |A| + |B| - |A \cup B|$\\
- $P(A^c) = 1 - P(A)$\\
\\
...
\\
\textbf{r-Combination:} The number of ways to choose $r$ objects from a set of $n$ distinct objects is given by:
- $nCr = \binom{n}{r} = \frac{n!}{r!(n-r)!}$\\
