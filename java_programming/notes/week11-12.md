...
### PriorityQueue
The ```PriorityQueue<T>``` class is a queue where each element has a priority. The elements are retrieved first in order of their priority then by FIFO.

### Stack
The standard stack implementation is in ```java.util.Stack```